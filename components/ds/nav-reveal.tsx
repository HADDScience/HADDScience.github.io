...
/**
 * NavReveal — 스크롤 스테이지 구간에서는 상단 네비를 숨기고, 스테이지를 지나면 다시 보인다.
 *
 * 왜 이 컴포넌트가 따로 있는가
 *   네비는 레이아웃에 있어 모든 페이지가 공유한다. 숨김 여부를 NavBar 안의 state 로 두면
 *   페이지 이동 때 값이 남아 다른 페이지에서 네비가 사라질 수 있다. 그래서 **스테이지가
 ...
 *   점프하면 네비가 숨지 않았다. IntersectionObserver 는 교차 상태가 *바뀔 때만* 발화한다.
...
export function NavReveal({
  ...
  React.useEffect(() => {
    ...
    body.dataset.navHidden = "true"

    const observer = new IntersectionObserver(
      ([entry]) => {
        body.dataset.navHidden = entry.isIntersecting ? "true" : "false"
      },
