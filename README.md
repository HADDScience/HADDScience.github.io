...
| 값 | 무엇 |
| --- | --- |
| 사진 | 2025 창업인큐베이팅 경진대회(중소벤처기업부 장관상 대상) · 우수사례 공모전 최우수상 등 **수상 기록 18장** |
| 톤 | `brightness 0.5 · saturation 0.72 · blur 6` + 네이비 스크림 alpha 0.30 (시안 B) |
| 대비 | 히어로 텍스트 영역 배경 평균 13.55:1 · 최악 9.83:1 (2026-09-22 실측, 1440×900) |
| 무게 | 타일 36장(와이드·세로) 합계 254KB. 첫 화면에 받는 것은 12장 |
...
- **18장이 전부 다른 사진이어야 한다.** 타일 번호는 세 장씩 끊어 칸에 들어가고 같은
  줄끼리 한 화면에 뜬다. 닮은 컷(단체 기념사진 둘)을 같은 줄에 두면 흐린 배경에서
  거의 같은 그림으로 보인다.
