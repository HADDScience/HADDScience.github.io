...
| `GOOGLE_SITE_VERIFICATION` | 구글 서치콘솔 소유확인 코드(`content=` 안쪽만). 없으면 태그가 아예 나가지 않는다 |
| `NAVER_SITE_VERIFICATION` | 네이버 서치어드바이저 소유확인 코드. 위와 같다 |
...
- 아이콘 세트가 없으므로 아이콘을 그리지 않는다. 번호 라벨(`01`) + 2px 블루 룰,
  Tag pill, 유니코드 `→ › ‹ +` 만 쓴다.
- 투명도 + 블러는 스티키 헤더에만.
- 사진 위에 얹는 배지는 채운 색이어야 읽힌다 — `Badge` 의 `brand` 톤(`bg-brand-blue-700`)은
  고정 카드용으로, 어떤 썸네일 위에서도 대비가 서게 반투명을 쓰지 않는다.
- 모션은 `cubic-bezier(.2,.6,.2,1)` 단일 커브. 색 120ms, 인터랙션 200ms.
- 자동 재생 캐러셀 금지 → 실제 사이트의 히어로 3장을 세로 밴드로 폈다.
  (예외는 히어로 배경 플립 그리드 하나다 — 아래 "배경" 참고. 바뀌는 것이 읽을 내용이 아니라 텍스처다.)
...
2026-09-22 에 한 장짜리 콜라주를 걷어내고 **여섯 칸이 각자 도는 플립 그리드**로 바꿨다
(`components/ds/hero-flip-grid.tsx` · 타일은 `scripts/build-hero-tiles.mjs`).
데스크톱 3열×2행, 모바일 2열×3행 — 둘 다 여섯 칸이다. 2.6초마다 **한 칸만**
`rotateX(180deg)` 로 뒤집히므로 한 칸은 약 15.6초마다 바뀐다.

| 값 | 무엇 |
| --- | --- |
| 사진 | 2025 창업인큐베이팅 경진대회(중소벤처기업부 장관상 대상) · 우수사례 공모전 최우수상 등 **수상 기록 18장 · 8개 장면** |
| 톤 | `brightness 0.5 · saturation 0.72 · blur 6` + 네이비 스크림 alpha 0.30 (시안 B) |
| 대비 | 설명 문단(흰 78%) 기준 **모든 타일 최저 5.89:1** — 빌드 스크립트가 타일마다 재고 4.5:1 미달이면 멈춘다 |
| 무게 | 타일 36장(와이드·세로) 합계 244KB. 첫 화면에 받는 것은 6장 + 다음 칸 1장 |

시안 네 개(A 깊은 블러 · B 얕은 블러 · C 네이비 듀오톤 · D 수상+활동 혼합)를 구워 비교하고
B 를 골랐다. 다시 비교하려면 `node scripts/build-hero-tiles.mjs --variant=c --out=/tmp/c` 로
따로 굽는다. 원본은 NAS 에 있고 레포에 넣지 않는다 — 경로는 `scripts/hero-tiles-sources.txt`
에 있다(`public/` 에 두면 배포 사이트로 NAS 경로가 노출된다).

> 디자인시스템은 "No patterns, no textures, no stock-photo backgrounds" 라고 못박고,
> 이 문서도 자동 재생 캐러셀을 금한다. 여기 쓰는 것은 스톡이 아닌 자사 기록이고,
> 자동으로 바뀌는 것은 **배경 텍스처뿐**이다 — 제목·버튼은 고정이므로 캐러셀처럼
> 읽을 내용을 놓치게 만들지 않는다. 히어로 한 곳에 한해 확장했다. 다른 곳에 사진 배경을
> 늘리지 말 것.

#### 같은 사진이 두 칸에 뜨지 않게 (2026-09-23)

처음에는 칸마다 사진 세 장을 고정했다. 파일은 18장 다 달랐는데, 연사로 찍힌 거의 같은
컷이 여럿이라 **흐리게 깔리면 같은 사진이 두 칸에 동시에 뜬 것처럼** 보였다. 지금은:

- 사진마다 **장면(`scene`)** 을 단다(`scripts/build-hero-tiles.mjs` 의 `PHOTOS`). 연사는
  한 장만 남기고, 구별은 되지만 닮은 컷(같은 무대 스크린 앞 · 같은 백드롭 앞 · 같은
  안내판)은 한 장면으로 묶는다.
- 칸마다 사진을 고정하지 않는다. 뒤집힐 칸에는 **지금 떠 있는 여섯 장과 장면이 다른
  사진** 중 가장 오래 안 나온 것을 **한 박자 전에** 넣어 둔다(`lib/hero-rotation.ts`).
  그래서 사진이 어느 칸에든 들어가고, 빌드 스크립트가 모든 타일을 제목 아래 칸 기준으로
  대비를 잰다.
- `node scripts/check-hero-rotation.mjs` 가 실제 목록으로 10만 번 뒤집어 확인한다 —
  모든 순간 장면이 서로 다름 · 뒤집을 때마다 장면이 바뀜 · 미리 넣어 둔 사진이 뜸.
  사진 목록을 바꾸면 다시 돌린다.

**장면 묶음은 흐린 타일을 나란히 놓고 눈으로 정한다.** 원본으로 판단하면 틀린다 —
원본에서 달라 보이던 「상장 사진」과 「테이블 사인」이 흐리게 보니 같은 대상 안내판이었고,
따로 두었더니 흰 안내판 두 장이 나란히 떴다. 장면은 7개 이상이어야 한다(떠 있는 6 +
다음 1). 스크립트가 확인한다.

지켜야 할 것 두 가지 더.
- **회전은 `<picture>` 에 건다.** `preserve-3d` 는 직계 자식에만 적용되므로 안쪽 `<img>` 에
  걸면 picture 가 평면으로 눌러 버려, 뒷면이 3D 로 돌지 않고 사진이 위아래로 뒤집힌 채
  그대로 보인다(첫 구현에서 인물이 거꾸로 섰다).
- **화면 밖·탭 숨김에서는 타이머를 멈춘다.** 보이지도 않는 배경 때문에 리페인트를
  계속 시킬 이유가 없다. `prefers-reduced-motion` 이면 아예 돌리지 않는다(첫 장 고정).

와이드/세로 두 벌을 `<picture>` 의 `source media` 로 갈랐다. next/image 두 개를
...
2026-09-22 에 정비했다. 등록 절차는 비개발자용 문서 [`docs/검색엔진-등록.md`](docs/검색엔진-등록.md).

| 무엇 | 어디 |
| --- | --- |
| canonical · hreflang · OG · 트위터 카드 | `lib/seo.ts` 의 `pageMetadata()` — 페이지마다 이것만 부른다 |
| 구조화 데이터 | 같은 파일. `Organization` · `WebSite`(레이아웃) · `BreadcrumbList` · `NewsArticle` · `Product` |
| 사이트맵 | `app/sitemap.ts` — URL 마다 `xhtml:link` 로 언어 짝을 함께 낸다 |
| RSS | `app/feed.xml/route.ts` — 한국어 기사 30건. 네이버가 사이트맵보다 자주 본다 |
| 소유확인 | 환경변수 두 개(위 표). **빌드 시점에 읽으므로 값을 넣은 뒤 재배포해야 태그가 나간다** |

지켜야 할 것.

- **canonical·hreflang 은 페이지별이다.** 2026-09-22 이전에는 레이아웃이 모든 페이지에
  `/ko`·`/en` 루트만 언어 짝으로 달아, 하위 페이지의 짝이 전부 틀린 곳을 가리켰다.
  레이아웃은 자기가 어느 경로에 얹혔는지 모른다 — 그래서 레이아웃에서 `alternates` 를 뺐다.
- **끝 슬래시를 맞춘다.** `next.config.ts` 가 `trailingSlash: true` 라 실제 주소가 `/ko/about/`
  이다. canonical 이 `/ko/about` 이면 자기 자신이 아닌 주소를 가리킨다.
- **페이지가 `openGraph` 를 주면 레이아웃의 것이 통째로 교체된다** (Next 의 메타데이터 병합은
  최상위 키 단위다). `pageMetadata()` 가 매번 `siteName`·`locale` 까지 다시 채우는 이유다.
- **목록 2페이지 이후는 `noindex, follow`** 다. 1페이지·기사와 같은 내용으로 경쟁하고, 글이
  늘면 같은 기사가 다른 페이지로 밀려 색인이 흔들린다. `follow` 는 켜 둬야 옛 기사까지 닿는다.
- **모르는 값은 구조화 데이터에 넣지 않는다.** 제품 가격·재고·평점, 회사 `sameAs` 가 그렇다.
  화면에 없는 것을 적으면 구글이 불일치로 본다.
- **키워드는 화면 카피에 실제로 있어야 한다.** `content/*.ts` 의 `meta.keywords` 는 그 말이
  본문에도 있다는 전제로 적은 것이다. 카피에서 말을 빼면 키워드도 같이 뺀다.

검색어별 전망은 `docs/검색엔진-등록.md` 5절에 적었다 — 「하드사이언스」·「haddscience」는
...
관리 화면은 이 통계를 **한 벌만 받아** 패널과 목록이 같이 쓴다(`hooks/use-visit-stats.ts`).
패널 안에서 따로 받아 오면 기간이 어긋난다 — 패널은 7일인데 목록의 글별 숫자는 30일치인 식이다.

글별 숫자는 목록의 각 줄에 붙고(`96회`, 방문이 없으면 `–`), **많이 본 순**으로 다시 세울 수 있다.
그 정렬은 화면에서만이다 — 사이트에 나가는 순서(`position`)는 건드리지 않는다.
한국어판과 영문판은 한 글로 합쳐 센다.
...
| 주소 | 무엇 |
| --- | --- |
| `/admin` | 콘텐츠 — 목록 · 검색 · 새 글 · 편집기. 줄마다 그 글의 조회 수 |
| `/admin/stats` | 통계 — 사이트 방문(방문자 · 추이 · 인기 · 유입) + 콘텐츠 현황(재고 · 손볼 것) |

`components/admin/shell.tsx` 가 로그인 관문과 머리·전환을 한 번만 다룬다. 페이지마다
로그인 상태 셋(`loading`·`anonymous`·`ready`)을 되풀이하면 한쪽만 고치는 일이 생긴다.

한 화면에 다 두지 않은 이유는, 글 하나 고치러 들어와도 통계부터 읽고 내려와야 했기 때문이다.
다만 **글별 조회 수는 콘텐츠 쪽에 남겼다** — 그건 그 글에 대한 정보라 관리하면서 같이 보는 편이 낫다.

통계 화면의 "손볼 것" 은 거기서 거르지 않고 **콘텐츠 화면으로 주소를 달고 넘어간다**
(`/admin?issue=no-body&tab=news`). 고칠 글은 고치는 화면에서 열어야 하고, 주소에 남으니
그 목록을 북마크하거나 남에게 보낼 수도 있다.
...
```jsonc
{
  "id": "20260827-1030",
  "date": "2026.08.27",
  "pinned": false,             // 목록 맨 위 고정. 카드에 HADD PICK 배지가 붙는다
...
목록 순서는 `position` 이다. 새 글은 0 으로 들어가고 나머지가 한 칸씩 밀린다. 그 위에
`pinned` 가 덧씌워져 고정한 글이 맨 앞으로 온다 — Omnis 가 `[pinned desc, position asc,
id desc]` 로 내려보내고 **사이트에는 정렬 코드가 없다**.

### 고정 (HADD PICK)

회사가 밀고 싶은 글을 목록 맨 위에 두고 그렇게 보이게 한다. 관리 화면 목록의 압정 버튼으로
켜고 끄며, 개수 제한 없이 뉴스·하드:라이브러리 각각에 쓴다.

화면은 **카드 크기를 그대로 두고** 2px 브랜드 블루 테두리 · `shadow-ds-brand` · `HADD PICK`
배지로 구분한다(줄형 목록에서는 날짜 옆 배지). 첫 줄만 가로로 넓게 쓰는 배치도 검토했으나
그리드 리듬이 깨지고 화면 폭에 따라 배치가 달라져 예측하기 어렵다.

배지 문구는 한글·영문이 같다(`ui.pinned`). 브랜드 표기라 번역하지 않는다.

토글은 기사 전체를 PUT 하지 않고 `PUT /posts/<id>/pin` 을 쓴다 — 본문·덱을 다시 쓰고 번역
검사까지 도는 것을 피하고, 화면이 들고 있던 옛 본문으로 남의 수정을 덮는 일도 막는다.
...
| **고정은 순서 이동이 아니다** | `pinned` 를 따로 두고 `position` 은 건드리지 않는다. 순서를 옮겨 흉내 내면 **고정을 풀었을 때 원래 자리를 알 수 없다.** 2026-09-22 오전에 손으로 `position 0` 으로 옮겼다가 같은 날 제자리로 되돌리고 고정만 얹었다. |
| **로그인 버튼을 없앴다** | 관리 화면 세션(localStorage)과 옴니스 로그인(다른 오리진의 쿠키)은 서로 읽을 수 없다. 확인하는 유일한 방법이 `/sso/authorize` 로 다녀오는 것이고 버튼이 하던 일이 그거라, 그냥 다녀온다. 무한 왕복은 `sessionStorage` 로 탭당 한 번으로 묶는다. |
...
| **현황은 목록에서 센다** | `components/admin/content-stats.tsx` 가 이미 받아 둔 목록(`/posts` 한 번)에서 숫자를 센다. 집계 API 를 따로 두지 않는다 — 화면의 목록과 다른 수가 나오는 것이 통계가 없는 것보다 나쁘고, 154건은 브라우저에서 세도 티가 나지 않는다. |
| **숫자는 누르면 필터** | "손볼 것"(번역 없음 · 본문 없음 · 썸네일 없음 · 요약 없음)은 누르면 목록이 그 글만 남는다. 할 일이 몇 건인지 알려 주고 그 글을 찾는 일은 사람에게 떠넘기면 숫자를 보여 준 보람이 없다. |

### 콘텐츠 현황 패널

목록 위의 요약이다. 타일 넷(탭 건수 · 카드뉴스 · 번역 있음 · 최근 12개월) · "손볼 것"
칩 · 월별 발행 막대. 지금 보고 있는 **탭 기준**이고, 탭을 옮기면 칩 필터는 풀린다
(결함 수가 탭마다 다르고, 0건인 필터가 켜진 채 넘어가면 빈 목록만 보인다).

결함 정의는 `matchesIssue()` 한 곳에 모아 두고 패널과 목록 필터가 같은 함수를 쓴다.
본문이 없는 글은 번역·요약 항목에서 뺀다 — 한 글이 칩 셋에 동시에 잡히면 무엇부터
하면 되는지가 사라진다.

막대는 한 갈래라 범례가 없고 브랜드 블루 한 색이다. 색을 나누면 없는 구분이 있는 것처럼
보인다. 글이 없는 달도 자리를 지키고(2px 회색 눈금) 값은 막대마다 hover 로 읽는다 —
12개 숫자를 다 적으면 읽히지 않는다. 관리 화면은 라이트 고정이라 다크 스텝은 없다.
